...
const EditPostPage = () => {
    ...
    const [img, setImg] = React.useState(state?.image);
    ...
    const handlePost = async (e) => {
        ...
        try {
            await axios.put(API_URL + "/posts/" + state.postId, data, { headers: authHeader() }).then(
                (res) => {
                    if (res?.status === 200) {
                        alert.show('Update post successfully!', {
                            timeout: 2000,
    ...
    return <>
        <form className="form-row login-form my-4" onSubmit={(e) => handlePost(e)}>
            <div className="d-flex align-items-center my-4">
                <h2 className="text-center fw-normal mb-0 me-3">Edit post</h2>
            </div>
...
export default adminLayout(EditPostPage);