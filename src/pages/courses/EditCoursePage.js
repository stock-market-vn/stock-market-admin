...
const EditCoursePage = () => {
    ...
    const [img, setImg] = React.useState(state?.urlBanner);
    const [authorName, setAuthorName] = React.useState(state?.authorName);
    const [title, setTitle] = React.useState(state?.title);
    const [content, setContent] = React.useState(state?.description);
    const [err, setErr] = React.useState();
    const [categories, setCategories] = React.useState([]);
    const [category, setCategory] = React.useState(state?.categoryId);
    const [authorAvt, setAuthorAvt] = React.useState(state?.authorAvatar);
    const [urlCourse, setUrlCourse] = React.useState(state?.urlCourse);
    ...
    const handlePost = async (e) => {
        ...
        try {
            await axios.put(API_URL + "/courses/"+state?._id, data, { headers: authHeader() }).then(
                (res) => {
                    if (res?.status === 200) {
    ...
    return <>
        <form className="form-row login-form my-4" onSubmit={(e) => handlePost(e)}>
            <div className="d-flex align-items-center my-4">
                <h2 className="text-center fw-normal mb-0 me-3">Edit course</h2>
            </div>
...
export default adminLayout(EditCoursePage);